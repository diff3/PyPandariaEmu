...
def catmull_rom(
    ...
    t3 = t2 * value
    return (
        _catmull_axis(float(p0.x), float(p1.x), float(p2.x), float(p3.x), value, t2, t3),
        _catmull_axis(float(p0.y), float(p1.y), float(p2.y), float(p3.y), value, t2, t3),
        _catmull_axis(float(p0.z), float(p1.z), float(p2.z), float(p3.z), value, t2, t3),
    )
...
def build_arc_lengths(nodes: tuple[MovementNode, ...], *, samples_per_segment: int = 12) -> tuple[ArcLengthSample, ...]:
    if len(nodes) < 2:
        return ()

    samples: list[ArcLengthSample] = [
        ArcLengthSample(0, 0.0, 0.0, int(nodes[0].map_id), float(nodes[0].x), float(nodes[0].y), float(nodes[0].z))
    ]
    total = 0.0
    previous = samples[0]
    ...
    for index in range(len(nodes) - 1):
        ...
        for step in range(1, segment_samples + 1):
            ...
            x, y, z = catmull_rom(p0, current, target, p3, t)
            sample_node = ArcLengthSample(index, t, 0.0, int(current.map_id), x, y, z)
            total += _sample_distance(previous, sample_node)
            sample = ArcLengthSample(index, t, total, int(current.map_id), x, y, z)
            samples.append(sample)
            previous = sample
...
def sample_by_distance(samples: tuple[ArcLengthSample, ...], target_distance: float) -> ArcLengthSample:
    if not samples:
    ...
    span = max(0.0001, float(current.distance) - float(previous.distance))
    ratio = max(0.0, min(1.0, (float(target_distance) - float(previous.distance)) / span))
